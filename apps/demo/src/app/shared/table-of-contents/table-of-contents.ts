...
export class TableOfContents implements OnInit, AfterViewInit, OnDestroy {
  ...
  addToc(sectionName: string, toc: TocItem[], sectionIndex = 0) {
    const links: Link[] = [];
    toc.forEach((header) => {
      const {top} = header.element.getBoundingClientRect();
      links.push({
        name: header.name,
        type: header.element.tagName.toLowerCase(),
        top: top,
        id: header.id || header.element.id || this.normalizeLinkName(header.name),
        active: false
      });
    });
  ...
  private normalizeLinkName(name: string): string {
    return name;
  }

  addHeaders(sectionName: string, docViewerContent: HTMLElement, sectionIndex = 0, selector: string = 'h3, h4') {
    const headers = Array.from<HTMLHeadingElement>(docViewerContent.querySelectorAll(selector));
    const links: Link[] = [];
